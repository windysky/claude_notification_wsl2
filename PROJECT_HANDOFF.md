...
| ExecutionPolicy Bypass for WSL remote paths (v1.3.2) | Completed | 2026-04-17 14:50 | `notify.sh` now passes `-ExecutionPolicy Bypass` to PowerShell. Fixes silent toast failure on fresh WSL installs where default `RemoteSigned` blocks scripts under `\\wsl.localhost\...`. |

## 4. Outstanding Work

No active work items. v1.3.2 is feature-complete.
...
**Starting point**: v1.3.2 is complete. The next time Claude Code launches, the new hooks in `~/.claude/settings.json` will take effect automatically (no restart of CC required for existing sessions to pick up; new sessions only).
